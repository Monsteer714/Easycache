...
void testHotDataAccess() {
    ...
    EasyCache::ArcCache<int, std::string> arc(CAPACITY, 2);
    EasyCache::EasyLfuCache<int, std::string> lfu_aging(CAPACITY, 20000);
    std::random_device rd;
    std::mt19937 gen(rd());

    // 基类指针指向派生类对象，添加LFU-Aging
    std::array<EasyCache::EasyCachePolicy<int, std::string>*, 5> caches = {&lru, &lfu, &arc, &klru, &lfu_aging};
...
void testLoopPattern() {
    ...
    EasyCache::EasyLruCache<int, std::string> lru(CAPACITY);
    EasyCache::EasyLfuCache<int, std::string> lfu(CAPACITY);
    EasyCache::ArcCache<int, std::string> arc(CAPACITY);
    // 为LRU-K设置合适的参数：
    // - 历史记录容量设为总循环大小的两倍，覆盖范围内和范围外的数据
    // - k=2，对于循环访问，这是一个合理的阈值
    EasyCache::EasyKLruCache<int, std::string> lruk(CAPACITY, LOOP_SIZE * 2, 2);
    EasyCache::EasyLfuCache<int, std::string> lfuAging(CAPACITY, 3000);

    std::array<EasyCache::EasyCachePolicy<int, std::string>*, 5> caches = {&lru, &lfu, &arc, &lruk, &lfuAging};
    std::vector<int> hits(5, 0);
    std::vector<int> get_operations(5, 0);
    std::vector<std::string> names = {"LRU", "LFU", "ARC", "LRU-K", "LFU-Aging"};
...
void testWorkloadShift() {
    ...
    EasyCache::EasyLfuCache<int, std::string> lfu(CAPACITY);
    EasyCache::ArcCache<int, std::string> arc(CAPACITY);
    EasyCache::EasyKLruCache<int, std::string> lruk(CAPACITY, 500, 2);
    EasyCache::EasyLfuCache<int, std::string> lfuAging(CAPACITY, 10000);
